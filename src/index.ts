...
type colorChecker = () => string 

let colorHexMaker: colorChecker; 
let colorRgbMaker: colorChecker;

colorHexMaker = () =>{
    let hexHash = '#'
    for(let i = 0; i < 6; i++){
        let hexSelector: number | string = hexArr[Math.floor(Math.random() * hexArr.length)]
        hexHash+=hexSelector
    }
    return hexHash
}
...
let hexMode: boolean = true;


btn.addEventListener('click', ()=>{
    let colorChanger: string;
    if(hexMode === true){
...
const rgbBtn = document.querySelector(".rgb-mode") as HTMLInputElement
const hexBtn = document.querySelector(".hex-mode") as HTMLInputElement
